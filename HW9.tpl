<html>
    ...
    <body>
        <form method="post">
            {if isset($type)}
                <p>{html_radios name='type' options=$type_array selected=$type}</p>
            {else}
                <p>{html_radios name='type' options=$type_array selected='1'}</p>
            {/if}
            <p>Ваше имя: <input type="text" name="name" value='{if isset($user_name)}{$user_name}{/if}'/></p>
        ...
        {if !isset($smarty.get.id)}
            {showAd}
        {else}
            <a href="HW9.php">Главная страница</a>
        {/if}
    </body>
</html>